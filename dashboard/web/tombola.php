...
/**
 * Jednoduché PDO připojení na tvůj MySQL
 * Případně si vytáhni z nějakého configu.
 */
$dsn      = 'mysql:host=localhost;dbname=quantum;charset=utf8mb4';
$dbUser   = 'quantum';
$dbPass   = 'QuantumHeslo1*'; // pozor na typ uvozovky v SQL create user
...
/**
 * URL na Quantum API – můžeš si přepnout env proměnnou
 * nebo nechat default.
 */
...
/**
 * Vrátí nové náhodné číslo lístku, které ještě nebylo použito v dané akci.
 * Používá Quantum API, případně fallback na PHP, když API padá.
 */
...
// POST logika – vytvoření akce + cen, losování, opakované losování
            ...
            if (!$currentEvent || !$currentPrize) {
                ...
                if ($wins >= (int)$currentPrize['quantity_total']) {
                    ...
                    if ($ticket === null) {
                        ...
                        $stmt->execute([$currentEventId, $currentPrizeId]);
